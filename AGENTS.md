...
- **Driver options (v0.10)**: `src/driver-options.ts` is the single place that composes selenium-webdriver Builder options from daemon flags (`--endpoint`, `--browser-args`, `--capabilities`, `--browser-binary`, `--driver-binary`, `--app-binary`, env binary overrides, browser profile/CDP). `src/daemon/server.ts` consumes the returned spec (including `usingServer` and `ServiceBuilder` for custom drivers). Unit tests: `tests/unit/driver-options.test.ts`. Browser names supported: chrome, edge, firefox, safari (macOS), electron (requires `--app-binary`).
- **Grid management (v0.10)**: `src/grid.ts` provides `parseShard`/`computeShardPlan` (round-robin sharding), `gridStatus()` (queries a Selenium Grid 4 hub's `/status`), and `formatGridStatus()`. CLI: `se-cli grid status|attach|distribute`. Unit tests: `tests/unit/grid.test.ts`.
