...
- **Driver options (v0.10)**: `src/driver-options.ts` is the single place that composes selenium-webdriver Builder options from daemon flags (`--endpoint`, `--browser-args`, `--capabilities`, `--browser-binary`, `--driver-binary`, env binary overrides, browser profile/CDP). `src/daemon/server.ts` consumes the returned spec (including `usingServer` and `ServiceBuilder` for custom drivers). Unit tests: `tests/unit/driver-options.test.ts`. Browser names supported: chrome, edge, firefox, safari (macOS).
- **Grid management (v0.10)**: `src/grid.ts` provides `parseShard`/`computeShardPlan` (round-robin sharding), `gridStatus()` (queries a Selenium Grid 4 hub's `/status`), and `formatGridStatus()`. CLI: `se-cli grid status|attach|distribute`. Unit tests: `tests/unit/grid.test.ts`; integration: `tests/integration/v0.10-grid.test.ts` (mock Grid 4 hub + real remote-session tests driven through a locally spawned chromedriver used as a Grid-node-equivalent W3C endpoint; the real-remote tests are gated by a probe and auto-skip where a child-spawned driver cannot create a session — GitHub Actions runners run them).
- **Safari support (v0.10)**: `--browser=safari` via safaridriver (macOS only, no headless/BiDi/CDP). Safari is skipped in `buildDriverSpec` option handling (no browserOptions needed) and in the daemon's BiDi `webSocketUrl` capability. Integration coverage lives in `tests/integration/v0.10-safari.test.ts` (8 tests over the safaridriver capability baseline), gated to `darwin` + `SE_CLI_TEST_SAFARI`; the CI macOS job runs it serially (`--no-file-parallelism`) because safaridriver allows one paired session at a time.
...
- **CI matrix**: integration matrix covers ubuntu/windows × chrome/edge/firefox (lifecycle + v0.10-remote + v0.10-grid) plus a macOS/safari job (v0.10-remote + v0.10-safari + v0.10-grid, run with `--no-file-parallelism` — safaridriver allows only one paired session). Safari lifecycle tests (CDP/emulation) are excluded because safaridriver does not support them.
